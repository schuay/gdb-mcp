...
| `list_sessions` | Show all active sessions with idle time |
...
`exec_command` handles execution commands correctly: `until`, `advance`,
`jump`, `signal`, and `return` all block until the inferior stops, just like
the named tools do.
